      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        {hero && (hero.title || hero.button_text) && (
          <div className="relative z-10 text-center max-w-4xl mx-auto px-6">
            {hero.title && <h1 className="text-6xl md:text-8xl font-black text-[#f0ede8] mb-4 tracking-tight leading-none">{hero.title}</h1>}
      ...
      <section className="py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          ...
          <div className="text-center mt-12 md:hidden">
            <Link href="/shop">
              <button className="px-8 py-3.5 bg-[#c9a84c] text-[#111] font-bold rounded-xl text-sm uppercase tracking-[0.1em]">View All Jerseys</button>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 md:py-32 border-y border-[#2e2d2d]">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="text-center mb-14">
            <p className="text-[10px] font-semibold tracking-[0.3em] uppercase text-[#c9a84c] mb-3">What We Offer</p>
            <h2 className="text-4xl md:text-5xl font-black text-[#f0ede8] tracking-tight">Pricing</h2>
          </div>
          {promoItem?.image_url ? (
            <div className="flex justify-center">
              <div className="relative rounded-2xl overflow-hidden border border-[#2e2d2d] shadow-[0_8px_60px_#00000060]"
                style={{ width: '100%', maxWidth: '480px', aspectRatio: '9/16' }}>
                ...
                {(promoItem.title || promoItem.discount_percentage) && (
                  <div className="absolute inset-0 bg-gradient-to-t from-[#111111cc] via-transparent to-transparent flex items-end p-8">
                    <div>
                      {promoItem.title && <h3 className="text-2xl font-black text-white mb-1">{promoItem.title}</h3>}
                      {promoItem.discount_percentage && <p className="text-[#c9a84c] font-bold text-lg">{promoItem.discount_percentage}% OFF</p>}
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="flex justify-center">
              <div className="rounded-2xl bg-[#1f1e1e] border border-[#2e2d2d] flex flex-col items-center justify-center gap-4"
                style={{ width: '100%', maxWidth: '480px', aspectRatio: '9/16' }}>
