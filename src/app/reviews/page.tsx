    ...
    <div className="min-h-screen pt-28 pb-20 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-10">
          ...
          <a href={REVIEW_LINK} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-8 py-4 bg-[#c9a84c] text-black font-bold rounded-xl hover:bg-[#b8943d] transition text-sm uppercase tracking-wide">Leave a Review</a>
        </div>

        {loading ? (
          <div className="text-center py-20 text-[#a09890]">Loading...</div>
        ) : reviews.length === 0 ? (
          <div className="text-center py-20 text-[#555]">No reviews yet.</div>
        ) : (
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
            {reviews.map((review) => (
              <div key={review.id} onClick={() => setLightbox(review.image_url)}
