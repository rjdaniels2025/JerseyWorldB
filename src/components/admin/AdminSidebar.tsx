...
export default function AdminSidebar() {
  ...
  return (
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#111]/97 backdrop-blur border-t border-[#222] flex items-center justify-around px-1 h-16">
        {links.map(({ href, label, icon: Icon }) => {
          const active = pathname === href
          return (
            <Link key={href} href={href}
              className={`flex flex-col items-center gap-0.5 px-1 py-1.5 rounded-xl transition-all min-w-0 ${
                active ? 'text-[#c9a84c]' : 'text-gray-600'
              }`}>
              <Icon size={17} />
              <span className="text-[8px] font-medium truncate max-w-[48px] text-center leading-tight">{label}</span>
            </Link>
          )
        })}
      </nav>
